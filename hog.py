...
def tail_points(opponent_score):
    ...
    if opponent_score == 0:
        return 1
    elif 0 < opponent_score < 10:
        return 2 * opponent_score + 1
    elif 10 <= opponent_score < 100:
        return 2 * abs(opponent_score // 10 - opponent_score % 10) + 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score0 in range(goal):
        ...
        for score1 in range(goal):
            compare_score = strategy(score0, score1)
            score = strategy(0, 0)
            if score is not compare_score:
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while i <= 10:
        averaged_dice = make_averaged(roll_dice, total_samples)(i, dice)
        if averaged_dice > the_average:
            the_average = averaged_dice
            result = i
        i += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    result = tail_points(opponent_score) + score
    if perfect_square(result) == True:
        return 0

    return tail_strategy(score, opponent_score, threshold, num_rolls)  # Remove this line once implemented.
    # END PROBLEM 11
